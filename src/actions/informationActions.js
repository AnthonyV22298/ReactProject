...
export const readLicense = () => {
    return dispatch => {
        ...
        adalApiFetch(axios,"https://sstack4.crm.dynamics.com/api/data/v9.1/dmv_driverslicenses(fda16cc2-85c4-ea11-a812-000d3a8e4ace)?$select=dmv_ispermit,dmv_licenseclassupdated,dmv_licenseexpdate,dmv_licenseeye,dmv_licensehair,dmv_licenseheight,dmv_licenseissuedate,dmv_licenseorgandonor,dmv_licenseweight,dmv_name,dmv_reactivationdate,_dmv_testholdingcontact_value,dmv_usstates,statecode",
        config).then(res => {
...
export const readCitations = () => {
    return dispatch => {
        dispatch(infoRequestPending());
        adalApiFetch(axios,"https://sstack4.crm.dynamics.com/api/data/v9.1/crefc_driving_histories(9c445cee-25e3-ea11-a815-000d3a8e4ace)?$select=_dmv_driving_history_contact_id_value,dmv_license_status,dmv_suspensions,dmv_total_points,dmv_total_points_date,dmv_total_points_state&$expand=dmv_driving_history_dmv_citations_driving_history($select=crefc_citationsid,dmv_citation_number,dmv_conviction_date,dmv_court,dmv_date_received,dmv_details,_dmv_driving_history_value,dmv_location,dmv_order_of_suspension,dmv_points,dmv_type_of_ticket)",
        config).then(res => {
...
export const readData = (viewToRead) => {
    return dispatch => {
            switch(viewToRead) {
                case INFO_LICENSE:
                    dispatch(readLicense());
                    break;
                case INFO_CITATIONS:
                    dispatch(readCitations());
                    break;
                default:
                    break;
            }
